...
const NeedsScreen = () => {
  ...
  const needsCategories = [
    { label: "Housing", icon: require("../assets/icons/Housing.png") },
    { label: "Utilities", icon: require("../assets/icons/Utilities.png") },
    { label: "Transportation", icon: require("../assets/icons/Transportation.png") },
    { label: "Groceries", icon: require("../assets/icons/Groceries.png") },
    { label: "Healthcare", icon: require("../assets/icons/Healthcare.png") },
    { label: "Education", icon: require("../assets/icons/Education.png") },
    { label: "Childcare & Family", icon: require("../assets/icons/Childcare & Family.png") },
    { label: "Insurance", icon: require("../assets/icons/Insurance.png") },
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      amount: -Math.abs(parseFloat(amount)), // ensure it's an expense
      note,
    ...
    try {
      ...
      if (response.ok) {
        console.log(`${editId ? "📝 Updated" : "✅ Added"} needs:`, data.transaction);
        // Show popup notification
        setPopupMessage(`Needs: ${amount} (${selectedLabel})`);
        setShowPopup(true);
        setTimeout(() => setShowPopup(false), 4000);
        // Store notification
        const notificationService = NotificationService.getInstance();
        notificationService.addNotification(`Needs: ${amount} (${selectedLabel})`, "Transaction");
        // Hide popup after 4s, close modal after 3s
        setTimeout(() => {
          closeModal();
          router.push("/home");
        }, 3000);
      } else {
        alert(data.msg || "Failed to save entry");
      }
    } catch (err) {
