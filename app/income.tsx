...
const IncomeScreen = () => {
  ...
  const [isTyping, setIsTyping] = useState(false); // ✅ Fix input freezing
  ...
  const incomeCategories = [
    { label: "Salary", icon: require("../assets/icons/Salary.png") },
    { label: "Investments", icon: require("../assets/icons/Investments.png") },
    { label: "Part-Time", icon: require("../assets/icons/Part-Time.png") },
    { label: "Bonus", icon: require("../assets/icons/Bonus.png") },
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...
      note,
      date: editId ? params.date : new Date().toISOString(), // 🔥 this line fixes it!
    };
    ...
    try {
      ...
      if (response.ok) {
        console.log(`${editId ? "📝 Updated" : "✅ Added"} income:`, data.transaction);
        // Show popup notification
        setPopupMessage(`Income: +${amount} (${selectedLabel})`);
        setShowPopup(true);
        setTimeout(() => setShowPopup(false), 4000);
        // Store notification
        const notificationService = NotificationService.getInstance();
        notificationService.addNotification(`Income: +${amount} (${selectedLabel})`, "Transaction");
        ...
        alert(data.msg || "Failed to save income");
      }
    } catch (err) {
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              if (selectedTab !== tab.label) {
                router.push(tab.route as "/income" | "/wants" | "/needs" | "/savings");
